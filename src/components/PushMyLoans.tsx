...
import { getLoanCounter, getLoan, repayLoan, liquidateLoan, fundLoan, calculateTotalRepayment, formatPC } from '@/lib/push-client';
import { LoanStatus } from '@/lib/push-config';

interface PushMyLoansProps {
  signer: ethers.Signer;
  address: string;
}

export default function PushMyLoans({ signer, address }: PushMyLoansProps) {
  ...
  const loadLoans = async () => {
    try {
      setLoading(true);
      const counter = await getLoanCounter();
      const loadedLoans = [];
      ...
      for (let i = 1; i <= Number(counter); i++) {
        try {
          ...
          if (
            ...
            const totalRepayment = loan.status === LoanStatus.Active ? await calculateTotalRepayment(i) : '0';
  ...
  const handleRepayLoan = async (loanId: number, amount: string) => {
    ...
    try {
      setLoading(true);
      await repayLoan(signer, loanId, amount);
  ...
  const handleLiquidateLoan = async (loanId: number) => {
    ...
    try {
      setLoading(true);
      await liquidateLoan(signer, loanId);
  ...
  const handleFundLoan = async (loanId: number, amount: bigint) => {
    if (!confirm(`Fund ${formatPC(amount)} PC?`)) return;

    try {
      setLoading(true);
      await fundLoan(signer, loanId, formatPC(amount));
              ...
              <div key={loan.id} className="bg-white rounded-xl shadow-lg p-6">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-semibold">{formatPC(loan.principalAmount)} PC</p>
                  </div>
                  <div>
                    <p className="text-gray-600">Collateral</p>
                    <p className="font-semibold">{formatPC(loan.collateralAmount)} PC</p>
                  </div>
                  <div>
