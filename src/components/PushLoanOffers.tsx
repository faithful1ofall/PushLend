...
import { createOffer, acceptOffer, getOfferCounter, getOffer, cancelOffer } from '@/lib/push-client';
import { formatPC, parsePC } from '@/lib/push-client';

interface PushLoanOffersProps {
  signer: ethers.Signer;
  address: string;
}

export default function PushLoanOffers({ signer, address }: PushLoanOffersProps) {
  ...
  const loadOffers = async () => {
    try {
      setLoading(true);
      const counter = await getOfferCounter();
      const loadedOffers = [];

      for (let i = 1; i <= Number(counter); i++) {
        try {
          const offer = await getOffer(i);
          if (offer.active) {
  ...
  const handleCreateOffer = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const durationSeconds = parseInt(maxDuration) * 24 * 60 * 60; // Convert days to seconds
      await createOffer(signer, amount, parseInt(interestRate) * 100, durationSeconds, parseInt(minCollateralRatio));
  ...
  const handleAcceptOffer = async (offerId: number, offerAmount: bigint, minCollateral: bigint) => {
    ...
    const collateral = prompt(`Enter collateral amount in PC (minimum ${formatPC(minCollateral * offerAmount / BigInt(10000))} PC):`);
    if (!collateral) return;

    try {
      setLoading(true);
      const durationSeconds = parseInt(duration) * 24 * 60 * 60;
      await acceptOffer(signer, offerId, durationSeconds, collateral);
  ...
  const handleCancelOffer = async (offerId: number) => {
    ...
    try {
      setLoading(true);
      await cancelOffer(signer, offerId);
