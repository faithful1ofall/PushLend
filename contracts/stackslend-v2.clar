;; StacksLend - P2P Lending Platform Smart Contract v2
;; Fixed version with proper STX transfer handling
...
(define-data-var liquidation-threshold uint u12000)
...
;; Create a loan request (borrower provides collateral upfront)
(define-public (create-loan-request (amount uint) (collateral-amount uint) (interest-rate uint) (duration uint))
  (let
    ...
    ;; Create loan record (pending status)
    (map-set loans
      { loan-id: loan-id }
      {
        borrower: tx-sender,
        lender: contract-owner,
...
(define-public (fund-loan (loan-id uint))
  (let
    ...
    (asserts! (is-eq (get status loan) status-pending) err-already-exists)
